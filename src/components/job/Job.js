      ...
      <div className="job-info border border-success">
        <h2 className="title card-header text-danger">
          {data.title.split(" ").map((word, index) => (
            <span key={`word${index}`}><span className="title-initial">{word[0]}</span>{word.slice(1)} </span>
          ))}
        </h2>
        <div className="card-body">
          <p className="company-and-location fw-bold">
            {data.company_and_location.split(" ").map((word, index) => (
            <span key={`word${index}`}><span className="cl-initial">{word[0]}</span>{word.slice(1)} </span>
          ))}
          </p>
          <p className="job-type">{data.type.split(" ").map((word, index) => (
            <span key={`word${index}`}><span className="type-initial">{word[0]}</span>{word.slice(1)} </span>
          ))}</p>
          <a
